 ...
 * - sleepModeAsAuto - boolean for Breville devices that use "sleep" mode instead of "auto". Maps 
 *                     HomeKit's AUTO mode to the device's sleep mode. Default is false.
 * - showLight -       boolean for enabling light control. Default is false.
 * - nameLight -       Allows customisation of the light control name. Default is 'Air Purifier Light'
 *
 ...
 *
 *
 * Sample configuration with light control:
 * {
 *    "name": "Living Room Air Purifier",
 *    "type": "AirPurifier",
 ...
 *    "showAirQuality": true,
 *    "showLight": true,
 *    "nameLight": "Air Purifier Display",
 *    "sleepModeAsAuto": true
 * }
 *
 */
class AirPurifierAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        if (this.device.context.noRotationSpeed) {
            ...
            switch (this.device.context.manufacturer) {
                 case 'Breville':
                     _fanSpeedLabels = {0: 'low', 1: 'mid', 2: 'high'};
                     this._rotationSteps = [...Array(3).keys()];
                     fanSpeedSteps = 3;
                     break;
                case 'Proscenic':
    ...
    _registerPlatformAccessory() {
        ...
        /* If configured to control the light, include that service */
        if (this.device.context.showLight) {
            this._addLightService();
        }
    ...
    /**
     * Method to add the Lightbulb service for controlling the display light.
     */
    _addLightService() {
        const {Service} = this.hap;
        const nameLight = this.device.context.nameLight || 'Air Purifier Light';
        this.log.info('Adding light control: %s', nameLight);
        this.accessory.addService(Service.Lightbulb, nameLight);
    }
    /**
    ...
    _registerCharacteristics(dps) {
        ...
        /* Ensure the light service existence aligns with the configuration */
        if (!lightService && this.device.context.showLight) {
            this._addLightService();
            lightService = this.accessory.getService(Service.Lightbulb);
        } else if (lightService && !this.device.context.showLight) {
        ...
        if (lightService) {
            const nameLight = this.device.context.nameLight || 'Air Purifier Light';
            this._checkServiceName(lightService, nameLight);
            
            characteristicLightOn = lightService.getCharacteristic(Characteristic.On)
                .updateValue(this._getLightOn(dps[DP_LIGHT]))
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (lightService && changes.hasOwnProperty(DP_LIGHT)) {
                /* Light state change */
                const newLightOn = this._getLightOn(changes[DP_LIGHT]);
                if (characteristicLightOn.value !== newLightOn) {
                    characteristicLightOn.updateValue(newLightOn);
                }
            }
         });
     }
    ...
    setLightOn(value, callback) {
        return this.setState(DP_LIGHT, !!value, callback);
    }
    ...
    setTargetAirPurifierState(value, callback) {
        ...
        switch (value) {
             ...
             case Characteristic.TargetAirPurifierState.AUTO:
                if (this.device.context.manufacturer == 'Breville') {
                    if (this.device.context.sleepModeAsAuto) {
